...
export class MultiplayerPowerUpPongGateway
	implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
	@WebSocketServer() server: Server;
	private logger: Logger = new Logger('MultiplayerPowerUpPongGateway');
	...
	afterInit(server: Server) {
		this.logger.log('WebSocket MultiplayerPowerUpPongGateway initialized');
	}
	...
	handleGameUpdate(room: Room) {
		...
		if (room.ball.x <= paddleWidth + ballSize + 4) {
			if (room.ball.y >= room.players[0].paddle && room.ball.y <= room.players[0].paddle + room.leftPaddleSize) {
				...
				room.hits += 1;
				this.hitCheck(room);
				if (room.powerUpType === PowerUpType.speedUp && room.hits === room.speedUpHits) {
					room.ball.vx *= 2;
				}
			}

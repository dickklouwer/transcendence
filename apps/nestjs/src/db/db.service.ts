  ...
  async getChatsFromDataBase(
    ...
    try {
      ...
      const Allmsg = await this.db
        .select({
          chatid: schema.chatsUsers.chat_id,
          title: schema.chats.title,
          image: schema.chats.image,
          lastMessage: schema.messages.message,
          time: schema.messages.sent_at,
        })
        .from(schema.chatsUsers)
        .innerJoin(
          schema.chats,
          eq(schema.chatsUsers.chat_id, schema.chats.chat_id),
        )
        .innerJoin(
          schema.messages,
          eq(schema.chats.chat_id, schema.messages.chat_id),
        )

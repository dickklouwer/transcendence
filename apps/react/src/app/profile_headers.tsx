    ...
    return (
        <div className='relative inline-block'>
            ...
            <button id='dropdownNotificationButton' onClick={toggleFriendsInbox} className=' px-2 py-1 rounded-lg hover:bg-blue-700 transition-all duration-150'>
                ...
                {numberOfRequests > 0 && <span className="absolute right-11 p-1 inline-flex items-center justify-center py-1 text-xs font-bold leading-none text-red-100 transform translate-x-1/2 -translate-y-1/2 bg-red-600 rounded-full">{numberOfRequests}</span>}
            </button>
    ...
    useEffect(() => {
      if (tempTokenFromParams) {
        ...
        if (token) {
            ...
            return; // Exit the effect after setting the token and redirecting
        }

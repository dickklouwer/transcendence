...
const MessageInbox: React.FC<MessageInboxProps> = ({ user_intra_id }) => {
    ...
    return (
        ...
        <Link href={'/chats'} className="flex items-center justify-between px-2 py-1 rounded-lg hover:bg-blue-700 transition-all duration-150">
            ...
            {numberOfUnreadChats > 0 && <span className={`absolute right-6 bottom-[-5px] inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none bg-red-600 rounded-full`}>{numberOfUnreadChats}</span>}
            {amountGameInvites > 0 && <span className={`absolute left-6 bottom-[-5px] inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none bg-blue-600 rounded-full`}>{amountGameInvites}</span>}
        </Link>
        </div>
    );
}

                            ...
                            <div className="flex bg-slate-950 border-white m-2 rounded-md items-center space-x-4 p-2 px-4 justify-between">
                                ...
                                <div className="flex">
                                    {friendsList.find((friend) => friend.user_id_receive === user.intra_user_id && friend.is_approved == false) ?
                                        ...
                                        :
                                        <DisplayBlockedOrInvite user={user} setIsSent={setIsSent} />}
                                </div>
                            </div>}
                    </div>
                ))}
...
const DisplayBlockedOrInvite = ({ user, setIsSent }: { user: ExternalUser, setIsSent: Dispatch<SetStateAction<boolean>>}) => {
    ...
    return (
        <button className=" bg-red-500 hover:bg-red-700 transition-all duration-300 p-2 rounded-md"
            disabled={true}
            >
           <svg className="flex w-11 h-11" fill="#ffffff" height="200px" width="200px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" ><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> 
